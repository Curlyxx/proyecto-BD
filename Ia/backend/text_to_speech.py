...
def eliminar_conversacion(conversacion_id):
    try:
        ...
        cursor.execute(
            "DELETE FROM mensajes WHERE conversacion_id = %s", (conversacion_id,))

        # Luego eliminar la conversación
        cursor.execute(
            "DELETE FROM conversaciones WHERE id = %s", (conversacion_id,))
...
def buscar_imagen(prompt):
    try:
        with DDGS() as ddgs:
            # Obtener múltiples resultados (hasta 10) para tener más opciones
            results = ddgs.images(prompt, max_results=10)

            # Si no hay resultados en absoluto
            if not results:
                print(f"No se encontraron imágenes para '{prompt}'.")
                return None

            print(f"Se encontraron {len(results)} imágenes para '{prompt}'")

            # Seleccionar una imagen al azar entre los resultados
            # Esto proporciona variedad en las respuestas para la misma consulta
            imagen_seleccionada = random.choice(results)
            imagen_url = imagen_seleccionada["image"]
...
def obtener_info():
    ...
    5. Procesar la respuesta usando primero el handler conversacional
    6. Si no es conversacional, buscar en Wikipedia
    ...
    if prompt_imagen:
        ...
        if resultado:
            contenido_respuesta = f"Aquí tienes una imagen relacionada con '{prompt_imagen}'"
            ...
            return jsonify({
                ...
                "contenido_audio": f"Aquí tienes una imagen relacionada con {prompt_imagen}",
                "imagen_url": resultado["url"],
                "prompt": prompt_imagen,
                "conversacion_id": conversacion_id
            })
        else:
            contenido_respuesta = f"Lo siento, no pude encontrar imágenes para '{prompt_imagen}'."
            ...
            return jsonify({
                "contenido": contenido_respuesta,
                "contenido_audio": f"No pude encontrar imágenes que coincidan con tu búsqueda de {prompt_imagen}.",
                "conversacion_id": conversacion_id
            })

    # =================== BLOQUE DE DETECCIÓN CONVERSACIONAL MEJORADO ===================
    # Comprobar directamente si el texto coincide con patrones de saludos comunes
    termino_lower = termino.lower().strip()

    # Verificar coincidencias directas con saludos y otras frases conversacionales comunes
    # que podrían ser mal interpretadas como búsquedas de información
    frases_problematicas = {
        ...
        "nos vemos": "despedidas",
        "gracias": "agradecimientos",
        "muchas gracias": "agradecimientos"
    }

    # Verificar coincidencia exacta con frases problemáticas
    tipo_conversacion = None
    for frase, categoria in frases_problematicas.items():
        if termino_lower == frase or termino_lower.startswith(frase + " ") or termino_lower.endswith(" " + frase):
            tipo_conversacion = categoria
            print(f"Coincidencia directa encontrada: '{frase}' -> {categoria}")
            break

    # Si no se encontró una coincidencia exacta, utilizar el handler conversacional normal
    if not tipo_conversacion:
        # Usar la instancia global de handler_conversacional
        if not hasattr(app, 'handler_conversacional_instance'):
            app.handler_conversacional_instance = handler_conversacional

        tipo_conversacion = app.handler_conversacional_instance.detectar_tipo_conversacion(
            termino)
        print(
            f"Tipo de conversación detectado por handler: {tipo_conversacion}")

    # Si se detectó una categoría de conversación, manejarla directamente
    if tipo_conversacion:
        # Obtener la respuesta adecuada según el tipo de conversación
        if tipo_conversacion.startswith("palabra_corta:"):
            palabra = tipo_conversacion.split(":")[1]
            respuesta = random.choice(
                handler_conversacional.palabras_cortas[palabra])
        elif tipo_conversacion in handler_conversacional.respuestas:
            respuesta = random.choice(
                handler_conversacional.respuestas[tipo_conversacion])
        else:
        ...
        # Guardar la respuesta conversacional
        guardar_mensaje(
            conversacion_id,
            'ai',
            respuesta,
            sentimiento=None  # No tenemos sentimiento para respuestas conversacionales
        )

        return jsonify(datos_respuesta)
    # =================== FIN BLOQUE DE DETECCIÓN CONVERSACIONAL ===================

    # Si llegamos aquí, no se detectó como conversacional, continuamos con el procesamiento normal
